...
| System-wide design / data flow / scaling / multi-service | `skills/think/SKILL.md` (Document Architecture Mode) |
...
| Security audit / vulnerability check / threat model | `skills/check/SKILL.md` (Specialist Review) |
| Architecture compliance / design-to-code validation | `skills/check/SKILL.md` (Specialist Review) |
| Document review / PDF / release notes | `skills/check/SKILL.md` (Document Review Mode) |
...
| Regression / broke after update / "used to work" | `skills/hunt/SKILL.md` (Bisect Mode) |
| PDF looks wrong / page break issue / font not rendering | `skills/hunt/SKILL.md` (Rendering Bug Mode) |
...
| Codify design rules / write a spec / document patterns | `skills/learn/SKILL.md` (Specification Writing Mode) |
...
2. **Architecture vs Feature**: If the request affects multiple services or global data flow → `/think` (Document Architecture Mode). If it's a single feature → `/think` (Lightweight/Full).
3. **Security vs Review**: If the focus is "will this break" or "is it clean" → `/check`. If the focus is "can this be hacked" or "leak data" → `/check` (Specialist Review).
