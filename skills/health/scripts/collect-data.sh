#!/usr/bin/env bash
# Collect Gemini CLI configuration data for health audit.
...
P=$(pwd)
SETTINGS="$P/.gemini/settings.local.json"
TIER="${1:-auto}"
PROJECT_KEY=$(printf '%s' "$P" | sed 's|[/_]|-|g; s|^-||')
CONVO_DIR="$HOME/.gemini/projects/-${PROJECT_KEY}"
...
count_local_skills() {
  local count=0
  if [ -d "$P/.gemini/skills" ]; then
    count=$(find -L "$P/.gemini/skills" -maxdepth 4 -name "SKILL.md" 2>/dev/null | while IFS= read -r f; do
...
list_rule_files() {
  if [ -d "$P/.gemini/rules" ]; then
    find "$P/.gemini/rules" -type f -name "*.md" 2>/dev/null | sort || true
  fi
}
...
collect_skill_descriptions_raw() {
  if [ -d "$P/.gemini/skills" ]; then
    grep -r "^description:" "$P/.gemini/skills" 2>/dev/null || true
  fi
  if [ -d "$HOME/.gemini/skills" ]; then
    grep -r "^description:" "$HOME/.gemini/skills" 2>/dev/null || true
  fi
}
...
echo "gemini_md_lines: $(count_file_lines "$P/GEMINI.md")"
...
echo "[2/10] GEMINI.md (global + local)..."
echo "=== GEMINI.md (global) ===" ; cat ~/.gemini/GEMINI.md 2>/dev/null || echo "(none)"
echo "=== GEMINI.md (local) ===" ; cat "$P/GEMINI.md" 2>/dev/null || echo "(none)"
...
echo "global_gemini_words: $(count_file_words "$HOME/.gemini/GEMINI.md")"
echo "local_gemini_words: $(count_file_words "$P/GEMINI.md")"
...
echo "[6/10] Nested GEMINI.md + gitignore..."
echo "=== NESTED GEMINI.md ==="
_NESTED_GEMINI=$(find "$P" -maxdepth 4 -name "GEMINI.md" -not -path "$P/GEMINI.md" -not -path "*/.git/*" -not -path "*/node_modules/*" 2>/dev/null || true)
if [ -n "$_NESTED_GEMINI" ]; then
  printf '%s\n' "$_NESTED_GEMINI"
else
  echo "(none)"
fi
echo "=== GITIGNORE ==="
_GITIGNORE_HIT=$(git -C "$P" check-ignore -v .gemini/settings.local.json 2>/dev/null || true)
...
echo "=== MEMORY.md ==="
if [ -f "$HOME/.gemini/projects/-${PROJECT_KEY}/memory/MEMORY.md" ]; then
  head -50 "$HOME/.gemini/projects/-${PROJECT_KEY}/memory/MEMORY.md"
else
  echo "(none)"
fi
...
_SKILL_FOUND=0
for DIR in "$P/.gemini/skills" "$HOME/.gemini/skills"; do
  [ -d "$DIR" ] || continue
  while IFS= read -r f; do
    [ -n "$f" ] || continue
...
if ! { for DIR in "$P/.gemini/skills" "$HOME/.gemini/skills"; do
  [ -d "$DIR" ] || continue
