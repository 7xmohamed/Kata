...
- Check settings.local.json for `model` fields. Valid model IDs follow the pattern `gemini-*` (e.g., `gemini-opus-4-6`, `gemini-sonnet-4-6`, `gemini-haiku-4-5-20251001`). Any non-`gemini-*` model ID (e.g., a provider-specific alias or outdated name) is [!] -- a wrong model name silently wastes the entire session with no output.
...
- Check GEMINI.md or hooks for dynamic timestamps/dates in system context, they break prompt cache
...
- For each critical rule in GEMINI.md NEVER/ALWAYS items, check if:
  1. GEMINI.md declares the rule: intent layer
...
- Flag rules that only exist in one layer -- single-layer rules are fragile:
  - GEMINI.md-only rules: Gemini may ignore them under context pressure
...
Verification checks:
- SIMPLE: No formal verification section required. Only flag if Gemini declared done without running any check.
- STANDARD+: GEMINI.md should have a Verification section with per-task done-conditions.
...
3. Missing local patterns: project-specific behaviors reinforced in conversation but missing from local GEMINI.md.
4. Missing global patterns: cross-project behaviors missing from ~/.gemini/GEMINI.md.
...
6. Anti-patterns: only flag what is directly observable:
   - Gemini declaring done without running verification
...
[BEHAVIOR: rules violated | repeated corrections | add to local GEMINI.md | add to global GEMINI.md | skill frequency | anti-patterns (tag each with confidence level)]
