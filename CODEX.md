...
Personal skill collection for Codex (Cursor). Eight skills covering the complete engineering workflow: think, design, check, hunt, write, learn, read, health. Now with advanced Architecture, Security, and Synthesis modes.
...
Each skill has a `SKILL.md` (loaded on demand by Codex). Supporting content lives in subdirectories. `skills/RESOLVER.md` is the human-readable index of "which trigger goes to which skill"; keep it in sync when you change a skill's scope.
