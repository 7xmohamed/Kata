...
verify-scripts:
	...
	echo "bash -n: ok"
	echo "py_compile: skip (no scripts)"
	bash skills/health/scripts/collect-data.sh auto >/tmp/kata-collect-data.out
	echo "collect-data: ok"
	rg -n "^=== CONVERSATION SIGNALS ===$$|^=== CONVERSATION EXTRACT ===$$|^=== MCP ACCESS DENIALS ===$$" /tmp/kata-collect-data.out
...
smoke-statusline:
	...
	grep -q '"used_percentage": 12' "$$tmpdir/.cache/kata-statusline/last.json"; \
...
smoke-statusline-installer:
	@set -e; \
		...
		bin_dir="$$tmpdir/bin"; \
		mkdir -p "$$home_dir/.gemini" "$$bin_dir"; \
		...
		printf '%s\n' '{invalid json' > "$$home_dir/.gemini/settings.json"; \
		...
		grep -q 'invalid json' "$$home_dir/.gemini/settings.json"; \
		test ! -f "$$tmpdir/brew.log"; \
		printf '%s\n' '{"theme":"dark"}' > "$$home_dir/.gemini/settings.json"; \
		...
		python3 -c "import json, sys; data=json.load(open(sys.argv[1])); assert data['theme'] == 'dark'; assert data['statusLine']['command'] == 'bash ~/.gemini/statusline.sh'" "$$home_dir/.gemini/settings.json"; \
		test -x "$$home_dir/.gemini/statusline.sh"; \
		test ! -f "$$tmpdir/brew.log"; \
...
smoke-verify-skills:
	@set -e; \
		...
		cp -R . "$$tmpdir/repo2"; \
		python3 -c "import json; p='$$tmpdir/repo2/.gemini-plugin/marketplace.json'; d=json.load(open(p)); d['plugins'].append({'name':'ghost','description':'x','version':'1.0.0','category':'development','source':'./skills/ghost','homepage':'https://example.com'}); open(p,'w').write(json.dumps(d, indent=2) + '\n')"; \
		...
		cp -R . "$$tmpdir/repo3"; \
		python3 -c "import json; p='$$tmpdir/repo3/.gemini-plugin/marketplace.json'; d=json.load(open(p)); [entry.update({'source':'./skills/read'}) for entry in d['plugins'] if entry['name']=='check']; open(p,'w').write(json.dumps(d, indent=2) + '\n')"; \
...
smoke-health:
	@set -e; \
		tmpdir=$$(mktemp -d); \
	convo_dir="$$tmpdir/.gemini/projects/-$(PROJECT_KEY)"; \
	mkdir -p "$$convo_dir"; \
